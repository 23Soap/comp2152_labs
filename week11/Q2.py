...
class PasswordChecker:
    ...
    def __init__(self):
        self.common_passwords = ["admin","password","123456","root","guest","letmain","welcome"]
        self.history = []
    ...
    def evaluate(self, password):
        if self.check_common(password):
            result = "WEAK {commonPassword}!"
        else:
            strenght = self.check_strength(password)
            score = sum(strenght.values())
            if score <= 1:
                result = "WEAK!"
            elif score == 2:
                result = "Medium!"
